...
public class pre_placa_asignada extends Pantalla {
    ...
    public pre_placa_asignada() {
        ...
        bot_busca.setValue("Busc.Avanzada");
...
//        aut_busca.setMetodoChange("filtrarSolicitud");
        ...
        bar_botones.agregarComponente(new Etiqueta("Solicitud:"));
...
//        bar_botones.agregarBoton(bot_limpiar);
        ...
        dia_dialogop.setTitle("CONFIRMAR ASIGNACIÓN ESPECIAL"); //titulo
        dia_dialogop.setWidth("20%"); //siempre en porcentajes  ancho
        dia_dialogop.setHeight("10%");//siempre porcentaje   alto 
        dia_dialogop.setResizable(false); //para que no se pueda cambiar el tamaño
        dia_dialogop.getBot_aceptar().setMetodo("aceptoValoreses");
        dia_dialogop.getBot_cancelar().setMetodo("cancelarValores");
        grid_pa.setColumns(4);
        ...
        dia_dialogoa.setTitle("PLACA ASIGNADA - ANULAR"); //titulo
        ...
        Boton botPlaca = new Boton();
        botPlaca.setValue("ASIG.ESPECIAL");
        botPlaca.setIcon("ui-icon-bookmark");
        botPlaca.setMetodo("asigEspecial");
        bar_botones.agregarBoton(botPlaca);
    }
...
//        tab_solicitud.getColumna("DESCRIPCION_SOLICITUD").setMayusculas(true);
    ...
    public void aprobacion1() {
        dia_dialogop.Limpiar();
        dia_dialogop.setDialogo(gridp);
        dia_dialogop.setDialogo(grid_pa);
...
//        Integer papel = Integer.parseInt(ser_Placa.getTipoDisponiblePP(Integer.parseInt(tab_detalle.getValor("IDE_TIPO_VEHICULO")), Integer.parseInt(tab_detalle.getValor("IDE_TIPO_SERVICIO"))));
        if (fisicap > 0) {
            ...
            if (!tab_dato.isEmpty()) {
                // Cargo la información de la base de datos maestra 
                identificacion = Integer.parseInt(tab_dato.getValor("IDE_APROBACION_PLACA"));
                ser_Placa.seleccionarESP(Integer.parseInt(tab_detalle.getValor("IDE_DETALLE_SOLICITUD")), Integer.parseInt(tab_detalle.getValor("IDE_tipo_vehiculo")),
                        Integer.parseInt(tab_detalle.getValor("IDE_tipo_servicio")), identificacion);
                aceptoValores1();
                dia_dialogop.cerrar();
                tab_detalle.actualizar();
                utilitario.agregarMensajeInfo("Placas Asignada", "");
            } else {
                utilitario.agregarMensajeInfo("Solicitud no existe en la base de datos ", "");
            }
        } else {
            utilitario.agregarMensajeError("No Existen Placas Disponibles", "");
        }
