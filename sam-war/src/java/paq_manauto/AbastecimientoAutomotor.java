...
public class AbastecimientoAutomotor extends Pantalla {
    ...
    public AbastecimientoAutomotor() {
        ...
        autCompleta.setAutoCompletar("SELECT a.abastecimiento_id,a.abastecimiento_fecha,a.abastecimiento_numero_vale,\n"
                ...
                + "WHERE a.abastecimiento_fecha is not null and a.abastecimiento_tipo_ingreso = 'K' or a.abastecimiento_tipo_ingreso = 'O' \n"// v.tipo_ingreso = 'A' or v.placa='0000000' 
        ...
        setTabla.setHeader("REPORTE DE REGISTROS - SELECCIONE PARAMETROS");
    ...
    public void dibujarRegistro() {
        ...
        tabTabla.setTabla("mvabactecimiento_combustible", "abastecimiento_id", 1);
                    System.out.println(autCompleta.getValor());
        ...
        tabTabla.getColumna("abastecimiento_cod_conductor").setCombo("SELECT cod_empleado,nombres FROM srh_empleado where estado = 1 or cod_empleado in (SELECT DISTINCT cod_empleado FROM srh_num_contratos where fecha_fin >= '2015-01-01' or fecha_fin is null) order by nombres");
    ...
    public void kilometraje() {
        if (tabTabla.getValor("abastecimiento_kilometraje").equals("S/K")) {
            tabTabla.setValor("abastecimiento_comentario", "Vehiculo con tacometro dañado");
            utilitario.addUpdate("tabTabla");
        } else {
            TablaGenerica tabDato = aCombustible.getVehiculo(Integer.parseInt(tabTabla.getValor("mve_secuencial")));
            if (!tabDato.isEmpty()) {
