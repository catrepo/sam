...
public class pre_reportes_abastecimiento extends Pantalla {
    ...
    public pre_reportes_abastecimiento() {
        ...
        cmb_anos.setCombo("select ano_curso, ano_curso as anio from conc_ano \n"
                + "where ano_curso >= (cast(to_char(now(), 'YYYY') as INTEGER)-1)\n"
        ...
        cmb_ano.setCombo("select ano_curso, ano_curso as anio from conc_ano \n"
                + "where ano_curso >= (cast(to_char(now(), 'YYYY') as INTEGER)-1)\n"
                + "order by ano_curso desc");
        ...
        cmb_placa1.setCombo("select (case when placa is NULL then codigo_activo when placa is not null then placa end ) as placa,\n"
                + "(case when placa is NULL then codigo_activo when placa is not null then placa end ) as codigo\n"
                + "from  mv_vehiculo");
        ...
        cmb_anio.setCombo("select ano_curso, ano_curso as anio from conc_ano \n"
                + "where ano_curso >= (cast(to_char(now(), 'YYYY') as INTEGER)-1)\n"
                + "order by ano_curso desc");
