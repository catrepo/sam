...
public class CertificadoFondos extends Pantalla {
    ...
    public CertificadoFondos() {
        ...
        setRegistro.setSql("SELECT c.id_codigo,c.numero_certificado,c.fecha_certificado,c.memorando,c.descripcion_proyecto,c.monto,e.nombres,c.estado\n"
                + "FROM cert_fondos AS c\n"
                + "INNER JOIN srh_empleado AS e ON e.cod_empleado = c.codigo_funcionario "
                + "where c.anio ="+utilitario.getAnio(utilitario.getFechaActual()));
        setRegistro.getColumna("id_codigo").setVisible(false);
        setRegistro.getColumna("numero_certificado").setLongitud(2);
        setRegistro.getColumna("memorando").setLongitud(35);
        setRegistro.getColumna("numero_certificado").setFiltroContenido();
        setRegistro.getColumna("descripcion_proyecto").setFiltroContenido();
        setRegistro.getColumna("descripcion_proyecto").setLongitud(70);
        setRegistro.getColumna("nombres").setLongitud(35);
    ...
    public void dibujaCertificado() {
        ...
        tabOrden.getColumna("codigo_funcionario").setCombo("SELECT e.cod_empleado,e.nombres FROM srh_empleado e INNER JOIN srh_cargos c ON e.cod_cargo = c.cod_cargo\n"
                + "WHERE e.estado = 1 and c.nombre_cargo like 'FUNCIONARIO%' OR c.nombre_cargo like 'PRESIDENT%' ORDER BY e.nombres");
        ...
        tabOrden.getColumna("cargo_funcionario").setCombo("select cod_cargo,nombre_cargo from srh_cargos where nombre_cargo like 'FUNCIONARIO%' OR nombre_cargo like 'PRESIDENT%' order by nombre_cargo");
        ...
        tabOrden.getColumna("director_presupuesto").setCombo("SELECT e.cod_empleado,e.nombres FROM srh_empleado e INNER JOIN srh_cargos c ON e.cod_cargo = c.cod_cargo where c.cod_cargo = 607");
        ...
        tabOrden.getColumna("id_periodo").setValorDefecto(String.valueOf(utilitario.getMes(utilitario.getFechaActual())));
        ...
        tabOrden.getColumna("id_periodo").setVisible(false);
    ...
    public void anular() {
        ...
        utilitario.addUpdateTabla(tabOrden, "estado", "");
    ...
    public void numero() {
        ...
        String numero = programas.maxCertificados(utilitario.getAnio(utilitario.getFechaActual()));
        String num;
        Integer cantidad = 0;
    ...
    public void cargarProyecto() {
        tabOrden.getColumna("codigo_proyecto").setCombo("SELECT id_codigo,nombre_proyecto FROM cert_proyectos order by nombre_proyecto ");
        utilitario.addUpdateTabla(tabOrden, "codigo_proyecto", "");//actualiza solo componentes
    }
    ...
    public void aceptoOrden() {
        switch (rep_reporte.getNombre()) {
            case "CERTIFICADO DE FONDOS":
                String cadena = "";
                TablaGenerica tabTabla = programas.getVerificarCertf(Integer.parseInt(tabOrden.getValor("id_codigo")));
                if (!tabTabla.isEmpty()) {
                    if (tabTabla.getValor("estado").equals("Ingresado")) {
                        cadena = "";
                    } else {
                        cadena = "ANULADO";
                    }
