    ...
    public void mostrarColumna1() {
        if (comboAcciones.getValue().equals("1")) {//Información que se subira
            if (comboParametros.getValue().equals("1")) {//Ingresos
                ...
                buscaColumna();
            } else if (comboParametros.getValue().equals("5")) {//Press. Iess
                buscaColumna();
            } else {
...
//            setMigraRoles();
            abrirDialogo();
            if (comboParametros.getValue().equals("4")) {
            } else if (comboParametros.getValue().equals("5")) {
            } else {
                abrirDialogo();
            }
    ...
    public void buscaColumna() {
        Integer id = 0, col = 0, col1 = 0;
        String cadena = "";
        if (comboParametros.getValue().equals("4")) {
            for (int i = 0; i < tabTabla.getTotalFilas(); i++) {
                TablaGenerica tabDato = mDescuento.getListaValores(tabTabla.getValor(i, "cedula"));
                if (!tabDato.isEmpty()) {
                    if (tabDato.getValor("id_distributivo").equals("1")) {
                        id = 86;
                        cadena = "92,93";
                        col = 18;
                        col1 = 14;
                    ...
                    mDescuento.setFondos(Integer.parseInt(tabDato.getValor("cod_empleado")), id, Integer.parseInt(tabDato.getValor("id_distributivo")),
                            col1, cadena, col);
                } else {
                    utilitario.agregarMensaje("Datos no encontrados", tabTabla.getValor(i, "cedula"));
                }
            }
            tabTabla.actualizar();
        } else if (comboParametros.getValue().equals("5")) {
            for (int i = 0; i < tabTabla.getTotalFilas(); i++) {
                ...
                if (!tabDato.isEmpty()) {
                    ...
                    mDescuento.setPressIess(Integer.parseInt(tabTabla.getValor(i, "ide_descuento")), tabTabla.getValor(i, "cedula"), Integer.parseInt(tabDato.getValor("id_distributivo")),
                            Integer.parseInt(cadena), Integer.parseInt(tabDato.getValor("cod_empleado")));
                } else {
                    utilitario.agregarMensaje("Datos no encontrados", tabTabla.getValor(i, "cedula"));
                }
            }
    ...
    public void setMigraRoles() {
        ...
        if (!tabDato.isEmpty()) {
            if (comboParametros.getValue().equals("1")) {//ingresos
                ...
                TablaGenerica tabPres = mDescuento.getPrestamos();
                if (!tabPres.isEmpty()) {
                    for (int i = 0; i < tabPres.getTotalFilas(); i++) {
                        TablaGenerica tabDatos = mDescuento.getConfirmaDatos(tabPres.getValor(i, "ano"), Integer.parseInt(tabPres.getValor(i, "ide_periodo")),
                                tabPres.getValor(i, "ide_empleado"), Integer.parseInt(tabPres.getValor(i, "id_distributivo_roles")));
                        if (!tabDatos.isEmpty()) {
                            mDescuento.setmigrarDescuento(tabPres.getValor(i, "ide_empleado"), Integer.parseInt(tabPres.getValor(i, "ide_periodo")), Integer.parseInt(tabPres.getValor(i, "id_distributivo_roles")), Integer.parseInt(tabPres.getValor(i, "ide_columna")), tabConsulta.getValor("NICK_USUA"), "valor_egreso", Integer.parseInt(tabPres.getValor(i, "ano")), Double.valueOf(tabPres.getValor(i, "valor")));
                        } else {
                            utilitario.agregarMensaje("Datos No Concuerdan en el Rol", tabPres.getValor(i, "nombres"));
                        }
                    }
                }
                utilitario.agregarMensaje("REGISTRO SUBIDO CON EXITO A ROLES", " ");
