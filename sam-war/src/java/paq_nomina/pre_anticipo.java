 ...
 * @author p-chumana
 */
public class pre_anticipo extends Pantalla {
    ...
    public void llenarDatosE() {//SOLICITANTE
        ...
        if (!tab_dato.isEmpty()) {
            ...
            if (utilitario.validarCedula(tab_anticipo.getValor("ci_solicitante"))) {
                TablaGenerica tab_dato1 = iAnticipos.empleadosCed(tab_anticipo.getValor("ci_solicitante"), anio, 2);//empleados
                if (!tab_dato1.isEmpty()) {
                    Double rmu = Double.valueOf(tab_dato1.getValor("liquido"));
                    if (rmu > 1) {
                        ...
                        tab_anticipo.setValor("rmu", tab_dato1.getValor("sueldo"));
                        tab_anticipo.setValor("rmu_liquido_anterior", tab_dato1.getValor("liquido"));
                        tab_anticipo.setValor("id_distributivo", tab_dato1.getValor("id_distributivo"));
...
//                        utilitario.agregarNotificacionInfo("SU REMUNERACION ANTERIOR NO LE PERMITE REALIZAR ANTICIPO", tab_dato1.getValor("total"));
                    ...
                    TablaGenerica tab_dato2 = iAnticipos.empleadosCed(tab_anticipo.getValor("ci_solicitante"), anio, 2);//trabajadores
                    if (!tab_dato2.isEmpty()) {
                        Double rmut = Double.valueOf(tab_dato2.getValor("liquido"));
                        if (rmut > 1) {
                            ...
                            tab_anticipo.setValor("rmu", tab_dato2.getValor("sueldo"));
                            tab_anticipo.setValor("rmu_liquido_anterior", tab_dato2.getValor("liquido"));
                            tab_anticipo.setValor("id_distributivo", tab_dato2.getValor("id_distributivo"));
...
//                            utilitario.agregarNotificacionInfo("SU REMUNERACION ANTERIOR NO LE PERMITE REALIZAR ANTICIPO", tab_dato2.getValor("total"));
    ...
    public void datosSolicitud() {
        TablaGenerica tabActual = iAnticipos.getMesActual(tab_anticipo.getValor("ci_solicitante"), utilitario.getAnio(utilitario.getFechaActual()), (utilitario.getMes(utilitario.getFechaActual())-1));
        if (!tabActual.isEmpty()) {
            tab_anticipo.setValor("rmu", tabActual.getValor("sueldo"));
            tab_anticipo.setValor("rmu_liquido_anterior", tabActual.getValor("liquido"));
            utilitario.addUpdate("tab_anticipo");
        } else {
            utilitario.agregarMensaje("No encontro datos", null);
        }
    }
...
//                                    tab_parametros.setValor("numero_cuotas_anticipo", String.valueOf(calcularMes(new GregorianCalendar(anios_a,(meses_a+1),dias_a),new GregorianCalendar(anof,mesf,diaf))));//-1
            ...
            if (tab_anticipo.getValor("cod_tipo").equals("7") || tab_anticipo.getValor("cod_tipo").equals("10")) {
                if (utilitario.getDia(tab_parametros.getValor("FECHA_ANTICIPO")) <= 15) {//VALIDACION POR DIA HASTA 10
                    if (Integer.parseInt(tab_parametros.getValor("numero_cuotas_anticipo")) >= 1 && Integer.parseInt(tab_parametros.getValor("numero_cuotas_anticipo")) <= 20) {
                        llenarFecha();
                        cuotas();
                    } else {
                        utilitario.agregarMensaje("Tiempo Maximo de Pago", "20 MESES");
                    }
                } else if (utilitario.getDia(tab_parametros.getValor("FECHA_ANTICIPO")) >= 16 && utilitario.getDia(tab_parametros.getValor("FECHA_ANTICIPO")) <= 31) {//VALIDACION POR DIAS DEL 11 AL 28  
                    if (Integer.parseInt(tab_parametros.getValor("numero_cuotas_anticipo")) >= 1 && Integer.parseInt(tab_parametros.getValor("numero_cuotas_anticipo")) <= 20) {
                        llenarFecha();
