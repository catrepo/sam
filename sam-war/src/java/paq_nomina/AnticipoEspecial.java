    ...
    public void llenarFecha() {
        ...
        if (utilitario.getDia(tabParametros.getValor("FECHA_ANTICIPO")) <= 15) {
            ...
            if (Integer.parseInt(tabParametros.getValor("numero_cuotas_anticipo")) == 12) {
                ...
                if (calculo1 < 0) {
                    if (Integer.parseInt(tabParametros.getValor("numero_cuotas_anticipo")) == 12) {
                        Integer anioss = 0, mss = 0;
                        if (utilitario.getMes(utilitario.getFechaActual()) != 1) {
                            anioss = utilitario.getAnio(utilitario.getFechaActual()) + 1;
                            mss = utilitario.getMes(utilitario.getFechaActual()) - 1;
                        } else {
                            if (utilitario.getDia(utilitario.getFechaActual()) <= 25) {
                                ...
                                if (utilitario.getMes(utilitario.getFechaActual()) == 1) {
                                    anioss = utilitario.getAnio(utilitario.getFechaActual());
                                    mss = utilitario.getMes(utilitario.getFechaActual()) + 1;
                                } else {
                                    mss = utilitario.getMes(utilitario.getFechaActual()) - 1;
                                }
                        ...
                        TablaGenerica tab_dato = iAnticipos.periodos(meses(mss), String.valueOf(anioss));
                        if (!tab_dato.isEmpty()) {
                            //fecha inicial
                            fecha = tab_dato.getValor("ide_periodo_anticipo");
                            tabParametros.setValor("ide_periodo_anticipo_inicial", fecha);
                            utilitario.addUpdate("tabParametros");
                            //fecha final
                            TablaGenerica tab_dato2 = iAnticipos.periodos(meses(Integer.parseInt(mes)), String.valueOf(anioss + 1));
                            if (!tab_dato2.isEmpty()) {
                                fecha = tab_dato2.getValor("ide_periodo_anticipo");
                                tabParametros.setValor("ide_periodo_anticipo_final", fecha);
                                utilitario.addUpdate("tabParametros");
