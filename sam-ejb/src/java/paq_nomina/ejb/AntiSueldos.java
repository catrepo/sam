...
//Busqueda en roles de datos de servidor que solicita por numero de cedula
    public TablaGenerica empleadosCed1(String cedula, Integer anio, Integer mes) {
        conPostgresql();
        TablaGenerica tabFuncionario = new TablaGenerica();
        conPostgresql();
        tabFuncionario.setConexion(conPostgres);
        tabFuncionario.setSql("select aa.*,  \n"
                + "(d.TOTALI-c.TotalE) as total from    \n"
                + "(select E.COD_EMPLEADO,e.cod_banco,cod_tipo,cod_cuenta,cod_grupo,e.numero_cuenta,e.cedula_pass,e.nombres,c.cod_cargo,c.nombre_cargo,r.valor AS RU,id_distributivo_roles    \n"
                ...
                + "where ano= " + anio + " \n"
                + "and  id_distributivo_roles in(1,2) \n"
                + "and ide_periodo=" + mes + " \n"
                + "and ide_columnas in (14,40) \n"
                + "and e.cedula_pass like '" + cedula + "' \n"
                + "order by p.ide_funcion) AS aa    \n"
                + "left join     \n"
                + "(select E.COD_EMPLEADO,SUM(r.valor) AS TOTALE \n"
                + "from srh_roles as r, prec_programas as  p, srh_empleado as e \n"
                + "where e.cod_empleado=r.ide_empleado \n"
                + "and  ano= " + anio + "    \n"
                + "and  id_distributivo_roles in(1,2) \n"
                + "and ide_periodo=" + mes + " \n"
                + "and ide_columnas in (33,22,21,1,7,4,8,6,9,5,2,13,39,3,11,10,111,12,71,59,53,46,72,56,84,74,57,73,47,80,85,48,50,108,55,51,52,106,112,91,110)   \n"
                + "and r.ide_programa=p.ide_programa and valor>0    \n"
                + "GROUP BY E.COD_EMPLEADO) as c    \n"
                + "on aa.COD_EMPLEADO=c.COD_EMPLEADO   \n"
                + "left join     \n"
                + "(select E.COD_EMPLEADO,SUM(r.valor) AS TOTALI \n"
                + "from srh_roles as r, prec_programas as  p, srh_empleado as e \n"
                + "where e.cod_empleado=r.ide_empleado \n"
                + "and  ano= " + anio + "    \n"
                + "and  id_distributivo_roles in(1,2) \n"
                + "and ide_periodo=" + mes + " \n"
                + "and ide_columnas in (86,14,92,93,19,18,20,40,125,45,75,76,89,98,99,100,102,101,107,115)   \n"
                + "and r.ide_programa=p.ide_programa and valor>0    \n"
    ...
    public TablaGenerica empleadosCed(String cedula, Integer anio, Integer mes) {
        ...
        tabFuncionario.setSql("select * from(\n"
                ...
                + "INNER JOIN srh_cargos AS c ON c.cod_cargo = e.cod_cargo )as a\n"
                + "left join \n"
                + "(SELECT codigo, cedula,nombre,fecha,sueldo,anio,mes,liquido\n"
                + "from srh_temp_rol) as b\n"
                + "on a.cedula_pass=b.cedula\n"
                + "WHERE cedula_pass LIKE '" + cedula + "'");
        tabFuncionario.ejecutarSql();
        desPostgresql();
        return tabFuncionario;
    }

    public TablaGenerica getMesActual(String cedula, Integer anio, Integer mes) {
        conPostgresql();
        TablaGenerica tabFuncionario = new TablaGenerica();
        conPostgresql();
        tabFuncionario.setConexion(conPostgres);
        tabFuncionario.setSql("SELECT codigo, cedula,nombre,fecha,sueldo,anio,mes,liquido\n"
                + "from srh_temp_rol\n"
                + "where cedula = '" + cedula + "' and anio = " + anio + " and mes = " + mes);
        tabFuncionario.ejecutarSql();
        desPostgresql();
        return tabFuncionario;
    }
    ...
    public TablaGenerica getDatosServidor(String cedula) {
        conOraclesql();
        TablaGenerica tabFuncionario = new TablaGenerica();
        conOraclesql();
        tabFuncionario.setConexion(conPostgres);
        tabFuncionario.setSql("select codtra,nomtra,cedciu,fecnac,fecing,suebas,tipctt,\n"
                ...
                + "from nodattra WHERE cedciu = '" + cedula + "'");
        tabFuncionario.ejecutarSql();
        desPostgresql();
        return tabFuncionario;
    }
