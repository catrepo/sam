...
public class SQLManauto {
    ...
    public TablaGenerica getNumeroCampos1(String nombre) {
        conPostgresql();
        TablaGenerica tabTabla = new TablaGenerica();
        conPostgresql();
        tabTabla.setConexion(conPostgres);
        tabTabla.setSql("SELECT COLUMN_NAME,ordinal_position,(SELECT count(*) As NumeroCampos FROM information_schema.columns WHERE table_name = '" + nombre + "'\n"
                + ") as cantidad\n"
                + "FROM information_schema.columns \n"
                + "WHERE table_name = '" + nombre + "'");
        tabTabla.ejecutarSql();
    ...
    public void setProyecto(String nombre) {
        String parametro = "insert into cert_proyectos (nombre_proyecto) values ('" + nombre + "')";
        conPostgresql();
        conPostgres.ejecutarSql(parametro);
        desPostgresql();
    }
