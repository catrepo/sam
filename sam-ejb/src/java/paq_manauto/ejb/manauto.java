...
public class manauto {
    ...
    public TablaGenerica getCertificado(String nombre) {
        conPostgresql();
        TablaGenerica tabPersona = new TablaGenerica();
        conPostgresql();
        tabPersona.setConexion(conPostgres);
        tabPersona.setSql("SELECT id_codigo,nombre_proyecto FROM cert_proyectos where nombre_proyecto like '" + nombre + "'");
    ...
    public void deleteProyecto(Integer anti) {
        String auSql = "delete from cert_proyectos where id_codigo =" + anti;
        conPostgresql();
        conPostgres.ejecutarSql(auSql);
        desPostgresql();
    }
    ...
    public TablaGenerica getAbastecimientos(Integer anio, Integer id) {
        conPostgresql();
        TablaGenerica tabPersona = new TablaGenerica();
        tabPersona.setConexion(conPostgres);
        tabPersona.setSql("SELECT * FROM abastecimiento_combustible_vt where anio= " + anio + " and mve_secuencial = " + id + " order by abastecimiento_fecha");
        tabPersona.ejecutarSql();
        desPostgresql();
        return tabPersona;
    }

        public TablaGenerica getAbastecimien(Integer anio,String tipo) {
        conPostgresql();
        TablaGenerica tabPersona = new TablaGenerica();
        tabPersona.setConexion(conPostgres);
        tabPersona.setSql("SELECT * FROM abastecimiento_combustible_vt where anio= " + anio + " and tipo_ingreso ='"+tipo+"' order by abastecimiento_fecha");
        tabPersona.ejecutarSql();
    ...
    public void setDeleteTemp() {
        String auSql = "delete from mvabastecimiento_temp";
        conPostgresql();
        conPostgres.ejecutarSql(auSql);
        desPostgresql();
    }

    public void setInsertTemp(Integer secuencia, Integer mve_secuencial, String numero_vale, String fecha, String conductor,
            String kilometraje, String galones, Double total, Integer anio, Integer mes, String combustible,
            String tipo_ingreso, String horasmes, String comentario, String dependencia, String valorhora) {
        String parametro = "insert into mvabastecimiento_temp (secuencia, mve_secuencial,abastecimiento_numero_vale,abastecimiento_fecha,abastecimiento_conductor,\n"
                + " abastecimiento_kilometraje,abastecimiento_galones,abastecimiento_total,abastecimiento_anio,abastecimiento_mes,abastecimiento_combustible,\n"
                + " abastecimiento_tipo_ingreso,abastecimiento_horasmes,abastecimiento_comentario,abastecimiento_dependencia,abastecimiento_valorhora)\n"
                + "values ("+secuencia+", "+mve_secuencial+",'"+numero_vale+"','"+fecha+"','"+conductor+"',\n"
                + "'"+kilometraje+"','"+galones+"',"+total+","+anio+","+mes+",'"+combustible+"',\n"
                + "'"+tipo_ingreso+"','"+horasmes+"','"+comentario+"','"+dependencia+"','"+valorhora+"')";
        conPostgresql();
        conPostgres.ejecutarSql(parametro);
        desPostgresql();
    }
